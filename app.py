...
try:
    ...
    # 本地独立运行时 audit.py 可能不在 sys.path, 用同目录兜底
    import sys
...
def apizero_lookup(barcode):
    """带付费 Key 查询 apizero。付费 PRO(barcode-gs1) 优先, 失败降级免费版(barcode-lookup)。

    返回 (info, tier):
      info: 解析后的商品信息 dict, 查不到为 None
      tier: 'paid'(用了付费 barcode-gs1) / 'free'(用了免费 barcode-lookup) / 'none'(无 Key 或都失败)
    """
    key = load_apizero_key()
    if not key:
        # 无 Key 也尝试免费版 barcode-lookup(不带 key, 依赖匿名额度)
        url = "https://v1.apizero.cn/api/barcode-lookup?barcode=%s" % urllib.parse.quote(barcode)
        try:
        ...
        return None, "none"
    # 付费优先
    paid_url = "https://v1.apizero.cn/api/barcode-gs1?code=%s&key=%s" % (
        urllib.parse.quote(barcode), urllib.parse.quote(key))
    try:
        req = urllib.request.Request(paid_url, headers={"User-Agent": "grocyscan/1.0"})
        with urllib.request.urlopen(req, timeout=12) as r:
            j = json.loads(r.read().decode("utf-8"))
        info = _parse_apizero(j)
        if info:
        ...
        pass
    # 降级免费版(带 key)
    free_url = "https://v1.apizero.cn/api/barcode-lookup?barcode=%s&key=%s" % (
        urllib.parse.quote(barcode), urllib.parse.quote(key))
    try:
        req = urllib.request.Request(free_url, headers={"User-Agent": "grocyscan/1.0"})
        with urllib.request.urlopen(req, timeout=12) as r:
            j = json.loads(r.read().decode("utf-8"))
        info = _parse_apizero(j)
        if info:
...
def lookup(barcode):
    ...
    apizero_used = "none"
    try:
        info, apizero_used = apizero_lookup(barcode)
        if info:
            suggest = {
                "name": info.get("name", ""),
...
# 位置切换: 位置条码 -> Grocy 位置ID
# 条码格式: 9691000000 + 位置ID(2位) + EAN13校验位, 由 gen_barcodes.py 生成
LOCATION_BARCODE_PREFIX = "9691000000"
DEFAULT_LOCATION_ID = 3  # 玄关
# 每台扫描枪(client)当前记忆的位置, 持久化到 /data/location_state.json
_location_state = {}
_LOCATION_STATE_FILE = os.path.join(BASE, "location_state.json")


def _load_location_state():
    global _location_state
    try:
        with open(_LOCATION_STATE_FILE, "r", encoding="utf-8") as f:
            _location_state = json.load(f)
    except Exception:
        _location_state = {}
...
def _save_location_state():
    try:
        with open(_LOCATION_STATE_FILE, "w", encoding="utf-8") as f:
            json.dump(_location_state, f, ensure_ascii=False)
    except Exception:
        pass


def _resolve_location_name(loc_id):
    """把位置ID转成名称, 用于提示。失败返回 '位置%s'."""
    try:
        st, _, raw = grocy("GET", "/api/objects/locations")
        if st == 200:
...
def _check_location_barcode(barcode, client):
    """若 barcode 是位置条码, 切换该扫描枪的记忆位置并返回 (True, 消息)。否则返回 (False, None)。"""
    global _location_state
    if not (barcode and barcode.startswith(LOCATION_BARCODE_PREFIX)):
        return False, None
    tail = barcode[len(LOCATION_BARCODE_PREFIX):]
    if len(tail) != 3 or not tail[:-1].isdigit():
        return False, None
    loc_id = int(tail[:-1])
    # 校验 EAN13 校验位
    digits12 = barcode[:12]
    total = sum(int(ch) * (3 if i % 2 == 1 else 1) for i, ch in enumerate(digits12))
    check = str((10 - total % 10) % 10)
    if check != barcode[12]:
        return False, None
    cs = (client or "").strip() or "default"
    _location_state[cs] = loc_id
    _save_location_state()
    name = _resolve_location_name(loc_id)
    return True, {"ok": True, "location_set": True, "location_id": loc_id,
                  "location_name": name, "message": "已切换到位置: %s" % name}


def _get_memory_location(client):
    """返回该扫描枪记忆的位置ID, 无则默认玄关。"""
    global _location_state
    if not _location_state:
        _load_location_state()
    cs = (client or "").strip() or "default"
    return _location_state.get(cs, DEFAULT_LOCATION_ID)
...
def device_handle(barcode, client, mode):
    ...
    若条码是位置条码(9691000000前缀) → 切换该扫描枪的记忆位置, 后续商品归入该位置。
    """
    barcode = (barcode or "").strip()
    if not barcode:
        return {"ok": False, "error": "条码为空"}
    # 位置切换条码优先处理
    is_loc, loc_msg = _check_location_barcode(barcode, client)
    if is_loc:
        audit.log_entry({
            "source": "hardware", "operation": "location",
            "barcode": barcode, "location_id": loc_msg.get("location_id"),
            "location_name": loc_msg.get("location_name", ""),
            "result": "ok", "detail": loc_msg.get("message", ""),
        })
        return loc_msg
    try:
        d = lookup(barcode)
    ...
    # 确定位置: 优先用该扫描枪记忆的位置, client 为数字ID时直接用它
    ...
    if cs.isdigit():
        ...
        mem_loc = _get_memory_location(client)
        if mem_loc:
            loc_id = mem_loc
        elif cs:
            for l in locs:
    ...
    apizero_used = d.get("apizero_used", "none")
    loc_name = ""
    for l in locs:
        if l.get("id") == loc_id:
            loc_name = l.get("name", "")
    ...
    if barcode in SPLIT_RULES:
        ...
        if spl is not None:
            ok, msg = spl
            audit.log_entry({
                "source": "hardware", "operation": "split",
                "barcode": barcode, "amount": 1, "location_id": loc_id,
                "location_name": loc_name, "apizero": apizero_used,
                "result": "ok" if ok else "fail", "detail": msg,
            })
    ...
    if d.get("found"):
        ...
        if ok:
            audit.log_entry({
                "source": "hardware", "operation": mode,
                "barcode": barcode, "product_id": d.get("product_id"),
                "product_name": d.get("name", ""), "amount": 1,
                "location_id": loc_id, "location_name": loc_name,
                "apizero": apizero_used,
                "result": "ok",
                "detail": ("入库: " if mode == "in" else "出库: ") + str(d.get("name", "")),
            })
        ...
        audit.log_entry({
            ...
            "location_id": loc_id, "location_name": loc_name,
            "apizero": apizero_used,
            "result": "fail", "detail": msg or ("Grocy HTTP %s" % st),
        })
    ...
    if not d.get("need_create"):
        audit.log_entry({
            "source": "hardware", "operation": mode,
            "barcode": barcode, "location_id": loc_id, "location_name": loc_name,
            "apizero": apizero_used,
            "result": "fail", "detail": d.get("error") or "条码不存在且无法建档",
        })
    ...
    if not name:
        audit.log_entry({
            ...
            "barcode": barcode, "location_id": loc_id, "location_name": loc_name,
            "apizero": apizero_used,
            "result": "fail", "detail": "apizero 未返回商品名称, 无法自动建档",
        })
        return {"ok": False, "error": "apizero 未返回商品名称, 无法自动建档, 请先用网页扫码建档"}
    if not loc_id:
        audit.log_entry({
            "source": "hardware", "operation": mode,
            "barcode": barcode, "apizero": apizero_used,
            "result": "fail", "detail": "未找到可用位置",
        })
    ...
    if not pid:
        audit.log_entry({
            "source": "hardware", "operation": mode,
            "barcode": barcode, "product_name": name, "location_id": loc_id,
            "location_name": loc_name, "apizero": apizero_used,
            "result": "fail", "detail": msg,
        })
    ...
    if ok:
        audit.log_entry({
            "source": "hardware", "operation": mode,
            "barcode": barcode, "product_id": pid, "product_name": name,
            "amount": 1, "location_id": loc_id, "location_name": loc_name,
            "apizero": apizero_used, "auto_created": True,
            "result": "ok",
            "detail": "已建档并" + ("入库: " if mode == "in" else "出库: ") + name,
        })
    ...
    audit.log_entry({
        ...
        "apizero": apizero_used, "auto_created": True,
        "result": "fail",
        "detail": "已建档, 但" + ("入库" if mode == "in" else "出库") + "失败: " + msg,
    })
    return {"ok": True, "product_id": pid, "name": name, "action": mode, "created": True,
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if u.path == "/api/create":
            ...
            if pid:
                audit.log_entry({
                    "source": "phone", "operation": "create",
                    "barcode": barcode, "product_id": pid, "product_name": name,
                    "location_id": loc, "result": "ok",
                    "detail": "建档: " + name,
                })
            ...
            audit.log_entry({
                ...
                "location_id": loc, "result": "fail", "detail": msg,
            })
        ...
        pname = ""
        try:
            st_p, _, raw_p = grocy("GET", "/api/objects/products/" + str(pid))
            if st_p == 200:
                pname = (json.loads(raw_p.decode("utf-8")) or {}).get("name", "")
        except Exception:
            pass
        lname = ""
        if loc_id:
            lname = _resolve_location_name(int(loc_id))
        audit.log_entry({
            "source": "phone", "operation": mode,
            "product_id": pid, "product_name": pname, "amount": amount,
            "location_id": loc_id, "location_name": lname,
            "result": "ok" if ok else "fail",
            "detail": ("入库" if mode == "in" else "出库") + ("成功" if ok else "失败: %s" % msg),
        })
